<script lang="ts">
    import { lang } from "$lib/stores/lang";
    import { onMount } from "svelte";

    const skills = [
        {
            name: "Adobe Premiere Pro",
            level: "95%",
            icon: `<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><rect x="2" y="2" width="20" height="20" rx="2" fill="#9999FF"/><path fill="#00005C" d="M10.15 8.42a2.93 2.93 0 00-1.18-.2 13.9 13.9 0 00-1.09.02v3.36l.39.02h.53c.39 0 .78-.06 1.15-.18.32-.09.6-.28.82-.53.21-.25.31-.59.31-1.03a1.45 1.45 0 00-.93-1.46zM19.75.3H4.25A4.25 4.25 0 000 4.55v14.9c0 2.35 1.9 4.25 4.25 4.25h15.5c2.35 0 4.25-1.9 4.25-4.25V4.55C24 2.2 22.1.3 19.75.3zm-7.09 11.65c-.4.56-.96.98-1.61 1.22-.68.25-1.43.34-2.25.34l-.5-.01-.43-.01v3.21a.12.12 0 01-.11.14H5.82c-.08 0-.12-.04-.12-.13V6.42c0-.07.03-.11.1-.11l.56-.01.76-.02.87-.02.91-.01c.82 0 1.5.1 2.06.31.5.17.96.45 1.34.82.32.32.57.71.73 1.14.15.42.23.85.23 1.3 0 .86-.2 1.57-.6 2.13zm6.82-3.15v1.95c0 .08-.05.11-.16.11a4.35 4.35 0 00-1.92.37c-.19.09-.37.21-.51.37v5.1c0 .1-.04.14-.13.14h-1.97a.14.14 0 01-.16-.12v-5.58l-.01-.75-.02-.78c0-.23-.02-.45-.04-.68a.1.1 0 01.07-.11h1.78c.1 0 .18.07.2.16a3.03 3.03 0 01.13.92c.3-.35.67-.64 1.08-.86a3.1 3.1 0 011.52-.39c.07-.01.13.04.14.11v.04z"/></svg>`,
            color: "#9999FF",
        },
        {
            name: "Adobe After Effects",
            level: "90%",
            icon: `<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><rect x="2" y="2" width="20" height="20" rx="2" fill="#D291FF"/><path fill="#00005B" d="M8.54 10.73c-.1-.31-.19-.61-.29-.92s-.19-.6-.27-.89c-.08-.28-.15-.54-.22-.78h-.02c-.09.43-.2.86-.34 1.29-.15.48-.3.98-.46 1.48-.13.51-.29.98-.44 1.4h2.54c-.06-.21-.14-.46-.23-.72-.09-.27-.18-.56-.27-.86zm8.58-.29c-.55-.03-1.07.26-1.33.76-.12.23-.19.47-.22.72h2.109c.26 0 .45 0 .57-.01.08-.01.16-.03.23-.08v-.1c0-.13-.021-.25-.061-.37-.178-.56-.708-.94-1.298-.92zM19.75.3H4.25C1.9.3 0 2.2 0 4.55v14.9c0 2.35 1.9 4.25 4.25 4.25h15.5c2.35 0 4.25-1.9 4.25-4.25V4.55C24 2.2 22.1.3 19.75.3zm-7.04 16.511h-2.09c-.07.01-.14-.041-.16-.11l-.82-2.4H5.92l-.76 2.36c-.02.09-.1.15-.19.14H3.09c-.11 0-.14-.06-.11-.18L6.2 7.39c.03-.1.06-.19.1-.31.04-.21.06-.43.06-.65-.01-.05.03-.1.08-.11h2.59c.07 0 .12.03.13.08l3.65 10.25c.03.11.001.161-.1.161zm7.851-3.991c-.021.189-.031.33-.041.42-.01.07-.069.13-.14.13-.06 0-.17.01-.33.021-.159.02-.35.029-.579.029-.23 0-.471-.04-.73-.04h-3.17c.039.31.14.62.31.89.181.271.431.48.729.601.4.17.841.26 1.281.25.35-.011.699-.04 1.039-.11.311-.039.61-.119.891-.23.05-.039.08-.02.08.08v1.531c0 .039-.01.08-.021.119-.021.03-.04.051-.069.07-.32.14-.65.24-1 .3-.471.09-.94.13-1.42.12-.761 0-1.4-.12-1.92-.35-.49-.211-.921-.541-1.261-.95-.319-.39-.55-.83-.69-1.31-.14-.471-.209-.961-.209-1.461 0-.539.08-1.07.25-1.59.16-.5.41-.96.75-1.37.33-.4.739-.72 1.209-.95.471-.23 1.03-.31 1.67-.31.531-.01 1.06.09 1.55.31.41.18.77.45 1.05.8.26.34.47.72.601 1.14.129.4.189.81.189 1.22 0 .24-.01.45-.019.64z"/></svg>`,
            color: "#D291FF",
        },
        {
            name: "Adobe Photoshop",
            level: "80%",
            icon: `<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><rect x="2" y="2" width="20" height="20" rx="2" fill="#31A8FF"/><path fill="#001833" d="M9.85 8.42c-.37-.15-.77-.21-1.18-.2-.26 0-.49 0-.68.01-.2-.01-.34 0-.41.01v3.36c.14.01.27.02.39.02h.53c.39 0 .78-.06 1.15-.18.32-.09.6-.28.82-.53.21-.25.31-.59.31-1.03.01-.31-.07-.62-.23-.89-.17-.26-.41-.46-.7-.57zM19.75.3H4.25C1.9.3 0 2.2 0 4.55v14.899c0 2.35 1.9 4.25 4.25 4.25h15.5c2.35 0 4.25-1.9 4.25-4.25V4.55C24 2.2 22.1.3 19.75.3zm-7.391 11.65c-.399.56-.959.98-1.609 1.22-.68.25-1.43.34-2.25.34-.24 0-.4 0-.5-.01s-.24-.01-.43-.01v3.209c.01.07-.04.131-.11.141H5.52c-.08 0-.12-.041-.12-.131V6.42c0-.07.03-.11.1-.11.17 0 .33 0 .56-.01.24-.01.49-.01.76-.02s.56-.01.87-.02c.31-.01.61-.01.91-.01.82 0 1.5.1 2.06.31.5.17.96.45 1.34.82.32.32.57.71.73 1.14.149.42.229.85.229 1.3.001.86-.199 1.57-.6 2.13zm7.091 3.89c-.28.4-.671.709-1.12.891-.49.209-1.09.318-1.811.318-.459 0-.91-.039-1.359-.129-.35-.061-.7-.17-1.02-.32-.07-.039-.121-.109-.111-.189v-1.74c0-.029.011-.07.041-.09.029-.02.06-.01.09.01.39.23.8.391 1.24.49.379.1.779.15 1.18.15.38 0 .65-.051.83-.141.16-.07.27-.24.27-.42 0-.141-.08-.27-.24-.4-.16-.129-.489-.279-.979-.471-.51-.18-.979-.42-1.42-.719-.31-.221-.569-.51-.761-.85-.159-.32-.239-.67-.229-1.021 0-.43.12-.84.341-1.21.25-.4.619-.72 1.049-.92.469-.239 1.059-.349 1.769-.349.41 0 .83.03 1.24.09.3.04.59.12.86.23.039.01.08.05.1.09.01.04.02.08.02.12v1.63c0 .04-.02.08-.05.1-.09.02-.14.02-.18 0-.3-.16-.62-.27-.96-.34-.37-.08-.74-.13-1.12-.13-.2-.01-.41.02-.601.07-.129.03-.24.1-.31.2-.05.08-.08.18-.08.27s.04.18.101.26c.09.11.209.2.34.27.229.12.47.23.709.33.541.18 1.061.43 1.541.73.33.209.6.49.789.83.16.318.24.67.23 1.029.011.471-.129.94-.389 1.331z"/></svg>`,
            color: "#31A8FF",
        },
        {
            name: "Adobe Illustrator",
            level: "75%",
            icon: `<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><rect x="2" y="2" width="20" height="20" rx="2" fill="#FF9A00"/><path fill="#330000" d="M10.53 10.73c-.1-.31-.19-.61-.29-.92-.1-.31-.19-.6-.27-.89-.08-.28-.15-.54-.22-.78h-.02c-.09.43-.2.86-.34 1.29-.15.48-.3.98-.46 1.48-.14.51-.29.98-.44 1.4h2.54c-.06-.211-.14-.46-.23-.721-.09-.269-.18-.559-.27-.859zM19.75.3H4.25C1.9.3 0 2.2 0 4.55v14.9c0 2.35 1.9 4.25 4.25 4.25h15.5c2.35 0 4.25-1.9 4.25-4.25V4.55C24 2.2 22.1.3 19.75.3zM14.7 16.83h-2.091c-.069.01-.139-.04-.159-.11l-.82-2.38H7.91l-.76 2.35c-.02.09-.1.15-.19.141H5.08c-.11 0-.14-.061-.11-.18L8.19 7.38c.03-.1.06-.21.1-.33.04-.21.06-.43.06-.65-.01-.05.03-.1.08-.11h2.59c.08 0 .12.03.13.08l3.65 10.3c.03.109 0 .16-.1.16zm3.4-.15c0 .11-.039.16-.129.16H16.01c-.1 0-.15-.061-.15-.16v-7.7c0-.1.041-.14.131-.14h1.98c.09 0 .129.05.129.14v7.7zm-.209-9.03c-.231.24-.571.37-.911.35-.33.01-.65-.12-.891-.35-.23-.25-.35-.58-.34-.92-.01-.34.12-.66.359-.89.242-.23.562-.35.892-.35.391 0 .689.12.91.35.22.24.34.56.33.89.01.34-.11.67-.349.92z"/></svg>`,
            color: "#FF9A00",
        },
    ];

    let mounted = $state(false);
    onMount(() => {
        mounted = true;
    });
</script>

<section id="skills" class="skills-section">
    <div class="container">
        <div class="skills-header animate-on-scroll">
            <span class="sub-title"
                >{$lang === "ar" ? "أدواتي وعملي" : "TOOLS & STACK"}</span
            >
            <h2 class="title-premium">
                {$lang === "ar"
                    ? "الخبرة التقنية الرفيعة"
                    : "Technical Mastery"}
            </h2>
            <p class="section-desc">
                {$lang === "ar"
                    ? "أدمج بين الفن والتقنية لتحويل كل لقطة إلى تجربة بصرية لا تُنسى."
                    : "Merging artistic vision with technical precision for cinematic excellence."}
            </p>
        </div>

        <div class="skills-grid">
            {#each skills as skill}
                <div class="skill-card glass animate-on-scroll">
                    <div class="skill-top">
                        <div
                            class="skill-icon-wrapper"
                            style="border-color: {skill.color}; background: {skill.color}15;"
                        >
                            {@html skill.icon}
                        </div>
                        <div class="skill-info">
                            <h3>{skill.name}</h3>
                            <span class="skill-percent">{skill.level}</span>
                        </div>
                    </div>
                    <div class="progress-wrap">
                        <div
                            class="progress-line"
                            style="width: {mounted ? skill.level : '0%'};"
                        >
                            <div class="line-glow"></div>
                        </div>
                    </div>
                </div>
            {/each}
        </div>
    </div>
</section>

<style>
    .skills-section {
        padding: 140px 0;
        background: transparent;
        position: relative;
    }

    .skills-header {
        text-align: center;
        margin-bottom: 80px;
    }

    .sub-title {
        color: var(--primary);
        font-weight: 800;
        letter-spacing: 5px;
        font-size: 0.85rem;
        display: block;
        margin-bottom: 20px;
    }

    .title-premium {
        font-family: "Alexandria", sans-serif;
        font-size: clamp(2.4rem, 5vw, 4rem);
        font-weight: 900;
        margin: 0 auto 25px;
        padding: 0.2em 0.3em 0.5em; /* Standard padding */
        line-height: 1.2;
        display: inline-block;
        overflow: visible;
        background: linear-gradient(
            110deg,
            var(--primary) 0%,
            #ffffff 25%,
            #ffffff 75%,
            var(--primary) 100%
        );
        background-size: 200% auto;
        -webkit-background-clip: text;
        background-clip: text;
        -webkit-text-fill-color: transparent;
        text-shadow: 0 0 30px rgba(0, 229, 255, 0.15);
        animation: textShine 6s linear infinite;
        vertical-align: top;
    }

    @keyframes textShine {
        to { background-position: 200% center; }
    }

    .section-desc {
        color: var(--text-muted);
        font-size: 1.15rem;
        max-width: 600px;
        margin: 0 auto;
        line-height: 1.7;
    }

    .skills-grid {
        display: grid;
        grid-template-columns: repeat(auto-fit, minmax(320px, 1fr));
        gap: 35px;
    }

    .skill-card {
        padding: 45px 35px;
        border-radius: 32px;
        transition: var(--transition);
        border: 1px solid var(--glass-border);
        background: radial-gradient(
            circle at top left,
            rgba(255, 255, 255, 0.03),
            transparent
        );
    }

    .skill-card:hover {
        transform: translateY(-12px);
        border-color: var(--primary);
        box-shadow: 0 30px 60px rgba(0, 0, 0, 0.5);
    }

    .skill-top {
        display: flex;
        align-items: center;
        gap: 25px;
        margin-bottom: 35px;
    }

    .skill-icon-wrapper {
        width: 65px;
        height: 65px;
        border-radius: 12px;
        display: flex;
        align-items: center;
        justify-content: center;
        padding: 12px;
        border: 1px solid rgba(255, 255, 255, 0.1);
        transition: var(--transition);
    }

    .skill-icon-wrapper :global(svg) {
        width: 100%;
        height: 100%;
    }

    .skill-info h3 {
        font-size: 1.25rem;
        font-weight: 800;
        margin-bottom: 8px;
        color: white;
    }

    .skill-percent {
        font-size: 0.95rem;
        color: var(--primary);
        font-weight: 800;
        letter-spacing: 1px;
    }

    .progress-wrap {
        height: 4px;
        background: rgba(255, 255, 255, 0.05);
        border-radius: 10px;
        overflow: hidden;
        position: relative;
    }

    .progress-line {
        height: 100%;
        background: linear-gradient(90deg, var(--primary), #ffffff);
        border-radius: 10px;
        transition: width 2s var(--ease-out-expo);
        position: relative;
        box-shadow: 0 0 15px var(--primary-glow);
        overflow: hidden;
    }

    .progress-line::after {
        content: "";
        position: absolute;
        top: 0;
        left: -100%;
        width: 100%;
        height: 100%;
        background: linear-gradient(
            90deg,
            transparent,
            rgba(255, 255, 255, 0.4),
            transparent
        );
        animation: data-feed 2s infinite linear;
    }

    @keyframes data-feed {
        0% {
            left: -100%;
        }
        100% {
            left: 100%;
        }
    }

    .line-glow {
        position: absolute;
        inset: 0;
        background: var(--primary);
        filter: blur(8px);
        opacity: 0.5;
    }

    @media (max-width: 768px) {
        .skills-section {
            padding: 90px 24px;
        }
        .skill-card {
            padding: 35px 25px;
        }
    }
</style>
